...
| 21 | In-place ICE restart on the agent: does a newer `webrtcbin` (≥ 1.26) or `webrtcsink` support it? | [spike](../agent/spikes/webrtcbin-probe/README.md), [docs/23](23-agent-core-architecture.md#offer-construction-and-renegotiation) | ADR-0007 spike (slice 6) / next GStreamer baseline bump | until then rung 2 is the `session-close{retry:true}` fallback: a media restart instead of an in-place restart |

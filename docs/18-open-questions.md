...
| 2 | Jetson/NVENC encoder adapter — when? | camera-streamer heritage | roadmap insert post-M6 | design partner demand decides. **Hardware now exists** (2026-09-24): the self-hosted runner is an RTX 2080 Ti, and `libgstnvcodec.so` is already in the dev image, so its elements register wherever the NVIDIA driver is. Still three things away from usable: the NVIDIA Container Toolkit (we map `/dev/dri`, which is not how NVIDIA reaches a container), an `EncoderAdapter` and a `media.encoder = "nvenc"` value (docs/23 knows only `auto｜vaapi｜software`), and a docs/14 row. Note a desktop card's NVENC is **not** a robot's path either — Jetson encodes through `nvv4l2h264enc`, not nvcodec — so this box would develop the adapter, not validate a budget |

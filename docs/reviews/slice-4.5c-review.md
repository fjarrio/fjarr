...
| CI, after the slice was pushed | `socket-drop`'s `census-baseline` counted **every** session pipeline on the robot rather than its own, so a neighbouring scenario whose teardown lagged — which on a 2-core runner it does — failed this one for someone else's work. My first guess was that the slice's second robot had added load; it had not, because CI names the services it starts and never starts that one | scoped to this session's id, like the `session_pipeline_trigger` check three lines above it always was. The message now also names a neighbour that is still winding down, so the next reader is not left guessing |

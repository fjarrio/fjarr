...
- CI's first run of this commit died with exit 139 in `fjarr-opsim`'s `no-answer` scenario *after* both checks passed, inside the operator's teardown (a webrtcbin that never got a remote description set to NULL on the hosted runner); the rerun passed and it does not reproduce locally. Same class as the teardown aborts slice 3c worked around by exiting after the verdict — that exit sits after `op.teardown()`, so a second sighting should move the verdict before the teardown for scenarios whose peer never negotiated.
